...
export default function board(
	...
	switch (type) {
		...
		const [listsIds, rawLists] = normalizeArrayOfObjectsByKey<BoardListRaw, 'id'>(payload.lists, 'id');
		const items: HashMap<BoardItem> = {};
		const lists: HashMap<BoardList> = {};

		listsIds.forEach(key => {
			const list = rawLists[key];
			const [itemsIds, listItems] = normalizeArrayOfObjectsByKey<BoardItem, 'id'>(list.items, 'id');
